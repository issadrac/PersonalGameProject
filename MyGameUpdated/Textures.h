...
public:
    ...
    void setWindowAndRenderer(SDL_Window* w, SDL_Renderer* r) {
        window = w;
        renderer = r;
        if (!window || !renderer) {
            cout << "WHAT";
    ...
    void toggleFullScreen(bool& isFullScreen, nlohmann::json scaleData, bool skip = false) {
        if (isFullScreen) {
            isFullScreen = false;
    ...
    void setScale(nlohmann::json scaleData) {
        if (height >= 1440) {
            scale = scaleData.at("scaleFor1440p");
        }
        else {
            scale = scaleData.at("scaleFor1080p");
        }
    }
