...
public:
	...
	vector<int> runAnimationX = { firstFrameX,secondFrameX,thirdFrameX,fourthFrameX };
	float gravity;
	float jumpPower;
	float velocityY;
	bool noClick;
	...
	MainCharacter() { // Initialize mainCharacter with all defaults
	...
	void fall(bool holdingUp = false) { // Makes the character fall
		if (velocityY >= MAX_SPEED) // Max velocity is 12
			velocityY = MAX_SPEED;
